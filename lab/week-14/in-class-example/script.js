...
async function showDetails() {
  ...
  const { title, name, overview, media_type, vote_average, backdrop_path } = movieSet[movieID];
  ...
  details.innerHTML = `
    <div id="detail-content">
        ...
        <iframe class="bkg-img"  src="https://www.youtube.com/embed/${video_id}?autoplay=1&showinfo=0&controls=0&mute=1&amp;start=5" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope;"></iframe>        </div>
        `;

  // <img src="http://image.tmdb.org/t/p/w1280${backdrop_path}" class="bkg-img" />
  details.addEventListener('click', () => {
    if (event.target.children.length && event.target.id !== 'detail-content') {
